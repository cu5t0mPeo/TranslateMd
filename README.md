...
If a user would like to receive these tokens on a different Spoke Chain, then they must take the additional step of bridging them over after they have received them on the other Spoke Chain.Cross-Chain Communication and Finality
...
Currently CCIP doesn’t allow you to specify the level of finality to wait for for the source chain transaction. Therefore we will by default use Wormhole for these types of transactions until the same has been implemented on CCIP.Cross-Chain Messaging Architecture
...
Routing a requested message to send through the GMP which was specified.
Forwarding a received message to the desired smart contract.
Storing failed messages to be retried/reversed.
...
The WormholeAdapter implements IBridgeAdapter and IWormholeReceiver. It accepts messages to send from BridgeRouter and uses the Wormhole Automatic Relayer to do so. It also receives messages sent from a different chain (which were sent from the Wormhole Automatic Relayer) and modifies the data to be in a common format for BridgeRouter.

The CCIPAdapter implements IBridgeAdapter and inherits from CCIPReceiver. It accepts messages to send from BridgeRouter and uses Chainlink Relayer to do so. It also receives messages sent from a different chain (which were sent from the Chainlink relayer) and modifies the data to be in a common format for BridgeRouter.
...
Sending a Message
To send a message, it is necessary to implement BridgeMessenger and call the _sendMessage function. It has parameters:
...
It ensures the same message cannot be received twice. 
On occasion, messages which fail in the handler are recorded so they can be later retried or used for recovery - see the following section for more details.
...
USDC Transfers
In the section “To Bridge or Not To Bridge”, we spoke about the hybrid approach we are taking when it comes to token transfers. Tokens such as USDC and other cross-chain native tokens will be bridged to the Hub Chain, while all other tokens will remain on their respective Spoke Chain.

We will use Circle CCTP to facilitate the Cross-Chain native USDC transfers.  Circle CCTP the depositForBurn function which burns USDC on the source chain and then mints USDC on the destination chain. The USDC minted will be transferred to the mintRecepient. The depositForBurnWithCaller has the restriction whereby only the destinationCaller address is able to call the function on the destination chain to mint the USDC.
...
The adapters check the message is from a known source chain + address, and it’s correctly formatted. It then forwards the message to BridgeRouter with the same parameters as we saw in Receiving a Message.
CCIP Adapter
The contract CCIPAdapter inherits from CCIPReceiver and overrides the function _ccipReceive(...). It receives the message in the format Client.Any2EVMMessage before converting it into the common format needed for BridgeRouter.
Wormhole Adapter
The contract WormholeAdapter implements IWormholeReceiver and the function receiveWormholeMessages(...). It receives the message as bytes before converting it into the common format needed for BridgeRouter.. 
...
We saw in the “USDC Transfers” section that we will also have a second version of the WormholeAdapter to support CCTP atomic transfers.
Hub Adapter
...
The RateLimiter is unaware of the price of a token so periodically the parameters of each bucket should be adjusted as to set an appropriate balance between usability and security.Account Management
...
It is intended for these addresses to be other smart contracts on the Hub Chain which use the Cross-Chain Lending Protocol under the hood. See the following section for more details.Lending Model
...
Utilisation Ratio
To determine the interest rates set we define the utilisation ratio, . It is a measure of how much of a pool has been borrowed compared to deposited. 


Borrow Interest Rates
To differentiate the variable parameters from the stable ones, we are going to use the subscript ‘s’ for stable and ‘v’ for variable. We also introduce the variable  to monitor the weight of the stable borrows compared to the total debt as a whole. 



The variable interest rate, , and stable borrow interest rate, , are then calculated based on an optimal utilisation ratio   and optimal stable to total debt ratio  set by the protocol. The constants  are parameters set by the protocol to manage the interest rates as well.
...
If , an excess is added to the stable borrow interest rate:
...
The deposit interest rate  is directly dependent on the overall borrow interest rate . Before we can calculate , we must first calculate the overall stable borrow interest amount ; the sum of the stable borrowed amounts multiplied by their respective interest rates.

Where:
 is the stable borrow amount
 is the stable borrow rate
the subscript  represents each stable borrow taken on the protocol.
...
For the deposit interest rate , the borrow interest is divided between the depositors, excluding what is retained by the protocol. The variable  is the retention rate i.e. the interest kept by the protocol as revenue.
...
To rebalance the stable rate down, the user’s loan stable rate  needs to be greater than the current stable rate  plus a delta, for instance .
...
Consider a user who deposits multiple assets and uses them as collateral for a loan. The borrowable amount  at time t for a given loan’s collateral balance is:


Where:
 denotes the current user collateral balance if they were to withdraw the fToken.
...
Borrow
After a user has locked up their collateral, they can borrow against it. The amount the user has borrowed of a given asset is defined as  whereas the borrow balance of a given asset  is defined as the amount borrowed plus accrued interest.
...
When a borrow is changed by an amount  e.g. a user increases or repays their existing borrow, the borrow balance updated:



The accrued interest  is defined as the difference between the borrow balance  and the borrowed amount :


When a user repays their borrow, a portion of the accrued interest is retained  by the protocol:


Where:
 is the retention rate i.e. the interest kept by the protocol as revenue.

When a user increases their borrow by an amount , the updated borrowed amount  is:



When a user repays their borrow by an amount , the updated borrowed amount  is:



When a user rebalances or switches between variable and stable rate borrow in either direction, the borrowed amount  remains the same:



If a user increases their stable rate borrow by an amount , their new interest rate is updated using a weighted average of the interest rate of their existing borrow   and the pool’s current stable borrow rate:


Where:
 

The borrow factor  represents the effective borrowed amount given a borrow, in percent terms. For example, a more volatile asset such as ALGO may have  (for $10 borrowed, it is as if $11 has been effectively borrowed) whereas USDC may have .

Consider a user who borrows multiple assets (variable and stable borrows are separate as well). The total borrowed amount  at time t for a given loan’s borrows is:
...
the subscript  represents each borrow of the loan where a borrow is a unique pairing of asset and variable/stable rate.

The total borrowed  must not exceed the borrowable amount , otherwise the loan can be liquidated by a third party:
...
To convert between a borrowed asset of amount  e.g. USDC and a collateral fToken amount, the following formula known a borrow to collateral  is used:

Where:
 is the collateral fToken amount
 is the price of the borrowed asset
 is the price of the collateral asset

To incentivise third parties to liquidate other users’ under-collateralized loans, a liquidation bonus  is earned. For example, if  and the liquidation amount  is $50, the collateral seized will be $55. If there is insufficient balance of the chosen collateral, then at most the full collateral balance will be seized. 
...
A portion of the liquidation bonus  is kept by the protocol as revenue, as determined by the liquidation fee . For example,  if , the liquidation amount is $50 and the collateral seized is $55, the liquidation bonus retained will be $1.
...
Note that this logic is drawn from the Oracle Manager of Synthetix v3 
Architecture
...
Process the feed from both Oracle1 and Oracle2.
For each oracle, check that their price isn’t too old, and if it is, then Oracle3 will be used in its place.
Check that the two feeds which came from the two nodes have different types.
Check the price deviation between the two feeds. 
This logic ensures a will be returned only if its underlying sources are up to date and at least two oracles return a similar price.Spoke Operations
...
There will be an instance of SpokeERC20 for each ERC20 token supported on the given Spoke Chain.Hub Operations
...
Liquidations
Liquidations are restricted to the Hub Chain to avoid issues asynchronicity of price updates and reversions. The liquidator does not repay a borrow but instead transfers the debt and seized collateral to them. Errors & Recovery
...
The ability to retry receiving the message.
The ability to reverse the operation.
...
A retryReceiveMessage function which, given a failing message id, attempts to receive the message again. This is useful if the error was based on a temporal condition. 
A reverseMessage function which, given a failing message id, sends a message back to the original sender indicating that the message failed to be received. The original sender can then return any locked funds back to the user.
